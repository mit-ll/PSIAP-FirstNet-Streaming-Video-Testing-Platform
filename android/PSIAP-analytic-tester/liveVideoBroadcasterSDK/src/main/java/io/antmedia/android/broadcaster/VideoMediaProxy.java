...
//    private Camera _camera;
    private final VideoMediaProxy.MediaHandler _handler;
    ...
    public VideoMediaProxy(String filepath) {
        this.mFilepath = filepath;
        this._file = new File(filepath);
        if (_file.exists()) {
            mFileUri = Uri.fromFile(_file);
            Log.i(TAG, "Created uri: " + mFileUri.toString());
            try {
                ...
                Log.e(TAG, "Couldn't init mediaplayer: " + ioe.getMessage());
            }
        }

        ht = new HandlerThread("Video Media Proxy Thread");
        ht.start();

        _handler = new VideoMediaProxy.MediaHandler(ht.getLooper());
        _signal.close();
        _handler.obtainMessage(OPEN_CAMERA, 0, 0).sendToTarget();
        _signal.block();
//        if (_camera != null) {
//            _handler.obtainMessage(SET_ERROR_CALLBACK, new VideoMediaProxy.ErrorCallback()).sendToTarget();
//        }
    }
...
//                        _camera.setPreviewTexture((SurfaceTexture) msg.obj);
                        Log.i(TAG, "Setting surface texture");
//                        Surface s = new Surface((SurfaceTexture) msg.obj);
...
//                        _camera.addCallbackBuffer((byte[])msg.obj);
                        Log.i(TAG, "Callback buffer message sent...");
                        break;
